...
const getAttributes = (xmlString, attributes, ignorable) => {
  ...
  b.split(' ').forEach(i => {
    if (!ignorable.includes(i)) ignorable.push(i);
  });
};
...
var generateNumbering = function (zip, numberings) {
  ...
  xmlString = xmlString + '<w:numbering ' + numberings.attributes.join(' ') + ' mc:Ignorable="' + numberings.ignorable.join(' ') + '">' + numberings.abstractNum.join('') + numberings.num.join('') + '</w:numbering>';
