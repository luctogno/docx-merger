...
const updateMediaRelations = (zip, media) => {
	...
	const arr = Object.keys(childNodes).map(node => childNodes[node]).filter(item => item.getAttribute).map(item => item.getAttribute('Id')).filter(id => id.slice(0, 3) == 'rId' && +id.slice(3)).map(id => +id.slice(3));
	let id = Math.max(...arr);
	let addRelStr = "";
